...
{ 
    ...
    private int port = 0;
    public static ArrayList<String> newMsg=new ArrayList<String>();
    ...
    public void run(){
        try{
            ...
            while(count<6){
                ...
                ClientHandler c=new ClientHandler(uname,socket);
                String username = in.readUTF();  //Reads the username from the player
                clients.put(username, c);
                c.start();
                System.out.println("Client "+socket.getInetAddress().getHostName() + ":"+ username +" accepted");
                count+=1;
            }
        }
        catch(Exception e){
    ...
    public static void sendToOneClient (String userName,String msg) throws IOException
    {
        try{
        ClientHandler c = (ClientHandler)clients.get(userName);
...
class ClientHandler extends Thread{
   ...
   public ClientHandler (String userName, java.net.Socket socket) throws IOException
   {
